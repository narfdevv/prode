...
export default function Forecast() {
  ...
  const [activePhase, setActivePhase] = useState<"Grupos" | "Eliminatorias">("Eliminatorias");
  const [activeGroup, setActiveGroup] = useState("Todos");
  const [activeMatchday, setActiveMatchday] = useState("Todos");
  const [activeKnockoutStage, setActiveKnockoutStage] = useState("Todos");
  ...
  const groupStages = useMemo(
    () => Array.from(new Set(matches.filter((m) => m.stage.startsWith("Grupo ")).map((m) => m.stage))).sort(),
    [matches],
  );
  const knockoutStages = useMemo(
    () => Array.from(new Set(matches.filter((m) => !m.stage.startsWith("Grupo ")).map((m) => m.stage))),
    [matches],
  );
  const matchdaysByMatch = useMemo(() => getMatchdaysByMatch(matches), [matches]);
  const filteredMatches = matches.filter((match) => {
    const isGroupMatch = match.stage.startsWith("Grupo ");
    ...
    return !isGroupMatch && (activeKnockoutStage === "Todos" || match.stage === activeKnockoutStage);
  });

  const eliminatoriasByStage = useMemo(() => {
    if (activePhase !== "Eliminatorias") return [];
  ...
  }, [activePhase, filteredMatches]);

  const handlePhaseChange = (phase: string) => {
    setActivePhase(phase as "Grupos" | "Eliminatorias");
    setActiveGroup("Todos");
    setActiveMatchday("Todos");
    setActiveKnockoutStage("Todos");
  };
  ...
  return (
    <AppLayout>
      ...
      <Tabs value={activePhase} className="mb-3" onValueChange={handlePhaseChange}>
      ...
      {activePhase === "Grupos" && (
        <>
          <Tabs value={activeGroup} className="mb-3" onValueChange={setActiveGroup}>
          ...
          <Tabs value={activeMatchday} className="mb-6" onValueChange={setActiveMatchday}>
            <TabsList className="flex-wrap h-auto gap-1">
      ...
      {activePhase === "Grupos" ? (
        ...
        <>
          <Tabs value={activeKnockoutStage} className="mb-6" onValueChange={setActiveKnockoutStage}>
            <TabsList className="flex-wrap h-auto gap-1">
              <TabsTrigger value="Todos">Todos</TabsTrigger>
          ...
          {!isLoading && filteredMatches.length === 0 ? (
            <div className="bg-white rounded-xl border shadow-sm p-8 text-center text-slate-500">
              No hay partidos de eliminatorias disponibles.
            </div>
          ) : activeKnockoutStage !== "Todos" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {filteredMatches.map(renderMatchCard)}
            </div>
          ) : (
            <div className="space-y-8">
              {eliminatoriasByStage.map(([stage, stageMatches]) => (
                <div key={stage}>
                  <h2 className="text-base font-bold text-slate-700 mb-3">{stage}</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
